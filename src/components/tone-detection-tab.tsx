...
import { Gauge, Loader2, Sparkles, Download, Upload, FileText } from "lucide-react";
  ...
  const handleDetectTone = () => {
    if (!inputText.trim() && !fileDataUri) {
      toast({
        ...
        description: "Please enter some text or upload a file to detect its tone.",
        variant: "destructive",
      });
      return;
    }
    ...
    startTransition(async () => {
      try {
        const detectionResult = await toneDetection({
          text: inputText,
